...
Original file is located at
    https://colab.research.google.com/drive/1v7kJ0tJpWrwa2FeNBR5G70XCxVfkNLO0
...
def clean(review):
  ### Start of Answer ###
  return re.sub('[^a-z| ]', '', review.lower() )
  ### End of Answer ###
...
vectorized_data = [[vectorize(review, vocabulary), score] for review, score in clean_data]
### Answer ends here ###
...
english_data = [row for row in vectorized_data if sum(row[0])>4]   #if sum 5 and greater keep it
...
for bin, score in shuffled_data:
  if (score < 4):
    X.append(bin)
    y.append(0)
  elif (score >8):
...
def train_test_split(X, y, train_size):
  ...
  for i in range(numETrain):
    X_train.append(X[i])
    y_train.append(y[i])
    
  for i in range(numETrain, size):
    X_test.append(X[i])
    y_test.append(y[i])
...
### Answer starts here ###
rfc = RandomForestClassifier(n_estimators = 10, max_features = 50 )
