# -*- coding: utf-8 -*-
"""AJAssignment 3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1B8415Re43M59JzBSlgQHF83SHD02ga5x
...
class PCA:
    ...
    def fit(self, x):
        ...
        self.mean_ = 1/len(x)*x.sum(axis=0) #average face array
        phi = x - self.mean_         #standardized face array (500 x 9504)
        D = phi @ (phi.T)      # let D = phi @ phi transpose (500 x 500) 
        eigenValues, eigenVectors = np.linalg.eig(D)     #D has 500 largest eigenvalues of covariance matrix C = phi transpose @ pho (9504 x 9504)
        self.eigenvalues_= eigenValues[: self.n_components_]    #keep only n eigenvalues
        
        
        DEigenVectors = eigenVectors[:,np.arange(self.n_components_)] #500 x n_componenet
        self.components_ = phi.T @ DEigenVectors # since (eigenvectors C)= A @ (eigencectors D) (9504x500)(500x n_comp)  = (9504 x n_comp)
    ...
    def transform(self, x):
        ...
        norm = x -self.mean_  
        return norm @ self.components_ #(500 x9504)(9504 x n_comp) = (500 x n_comp)
...
subspace = pca.fit_transform(images)
...
# of an image.
eig_faces = pca.components_[:,np.arange(10)].T #(10 x 9504)  each row is eigenface

i=0
for face in eig_faces:
  eig_F = face.reshape(img_height, img_width)
  norm_eig_F = cv2.normalize(eig_F, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
  
  s1= './eigenfaces/eigenface'
  s2='.png'
  cv2.imwrite(s1+str(i)+s2, norm_eig_F)
  i += 1
...
print('Eigenfaces')
for img in glob.glob('./eigenfaces/eigenface[0-9].png'):
...
transformed_training_matrix = pca.transform(images)    #(500 x 50)
transformed_test_img = (test_img - pca.mean_ ) @ pca.components_  #(1, 9504) x (9504, 50) = (1,50)
...
distance = []
for face in transformed_training_matrix:
 distance.append(np.linalg.norm(face-transformed_test_img))

ranking = distance.index(min(distance))
